...
const links = ideasItems.slice(0, 3).map((item) => ({
    icon: <ArrowModal />,
    title: item.title,
    href: item.href,
}))
...
const MainLinks = ({ command, setCommand }: { command: number | null, setCommand: Dispatch<SetStateAction<number | null>> }) => {
    return (
        <motion.div 
...
export const CommandModal = () => {
    ...
    const increment = () => {
        if (command === null) {
            ...
            const filterCondition = command as number + 1 > links.filter(link => link.title.toLowerCase().includes(value.toLowerCase())).length - 1
            const notFitlerCondition = command as number + 1 > links.length + linksSocial.length - 1
            const condition = value.length > 0 ? filterCondition : notFitlerCondition

            setCommand(
                condition ? 0 : command as number + 1
            )
        }
    ...
    const decrement = () => {
        if (command === null) {
            ...
            const filterCondition = links.filter(link => link.title.toLowerCase().includes(value.toLowerCase())).length - 1
                ...
                <div className={styles.results}>
                    ...
                    ? (
                        <motion.div 
                            ...
                            transition={{ duration: 0.3, ease: 'easeInOut' }}
                        >
                            {ideasItems.filter((item) => item.title.toLowerCase().includes(value.toLowerCase())).map((item, index) => (
                                <ModalLink
                                    icon={<ArrowModal />}
                                    title={item.title}
                                    key={index}
...
const CommandButton = () => {
    ...
    return (
        ...
        <div className={styles.command}>
            <button className={styles.commandButton} onClick={() => setIsOpen(!isOpen)}>
                <Command />
            </button>  
        </div>
